...
class FemFileReader:
    ...
    def create_entities(self, parallel: bool = True):
        ...
        if parallel:
            chunks = modelReaderUtilities.get_chunks(
                self.file_content[self.endGridLine :]
            )
            ...
            for result in futures:
                ...
                element_mpc_id_config_master_nodes_dofs += result[2]
        else:
            # Process all at once
            (
                element_1d_id_prop_node1_node2,
        ...
        for (
            ...
            nodes,
            dofs,
        ) in element_mpc_id_config_master_nodes_dofs:
    ...
    def __process_1D_element(self, line_content: List[str]) -> Tuple:
        ...
        return (element_id, property_id, node1, node2)
    ...
    def __process_3D_element(
        ...
        while i + 1 < len(chunk) and chunk[i + 1].startswith("+"):
            ...
            node_ids += modelReaderUtilities.split_line(chunk[i], self.blocksize)[1:]

        node_ids = [
            node_id.replace("+", "").strip() for node_id in node_ids if node_id.strip()
        ]

        nodes = [self.nodes_id2node[int(node_id)] for node_id in node_ids]
        return (element_id, property_id, nodes), i
    ...
    def __process_MPC_element(
        ...
        node_ids = [int(node) for node in node_ids if "." not in node and node.strip()]
        # cast to Nodes
    ...
    def __process_element_chunk(self, chunk: List[str]) -> List:
        ...
        for i, line in enumerate(chunk):
            ...
            if (
                not line.startswith(tuple(FemFileReader.keyword_3d_elements))
                and not line.startswith(tuple(FemFileReader.keyword_1d_elements))
                and not line.startswith(tuple(FemFileReader.keywprd_mpcs))
            ):
                continue

            if len(line) < 2 * self.blocksize:
                continue
            ...
            if element_keyword in FemFileReader.keyword_1d_elements:
                element_1d_id_prop_node1_node2.append(
                    self.__process_1D_element(line_content)
                )
            elif element_keyword in FemFileReader.keyword_3d_elements:
                element_info, i = self.__process_3D_element(line_content, chunk, i)
                element_3d_id_prop_nodes.append(element_info)

            elif element_keyword in FemFileReader.keywprd_mpcs:
                element_info, i = self.__process_MPC_element(line_content, chunk, i)
                element_mpc_id_config_master_nodes_dofs.append(element_info)
